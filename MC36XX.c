...
uint8_t MC36XXreadRegister8(uint8_t reg)
{
    uint8_t value;
		...
		iic_send(MC36XX_CFG_I2C_ADDR, &reg, 1, true);
		iic_read(MC36XX_CFG_I2C_ADDR, &value, 1);
    return value;
}
...
{
		iic_send(MC36XX_CFG_I2C_ADDR, &reg, 1, true);
		iic_read(MC36XX_CFG_I2C_ADDR, buffer, len);
}
...
void MC36XXwriteRegister8(uint8_t reg, uint8_t value)
{
	  uint8_t sendbuf[2] = {reg, value};
		iic_send(MC36XX_CFG_I2C_ADDR, sendbuf, 2, false);
}
...
void MC36XXreset()
{
    MC36XXwriteRegister8(0x10, 0x01);  //Go to standby.

    delay(2);

    MC36XXwriteRegister8(0x24, 0x40);  //Power on reset.
